...
# load the class file
num_classes = 4 #80
classes = load_classes("data/waymo.names") #coco
...
#Detection phase
try:
    imlist = [osp.join(images, img) for img in os.listdir(images)] #osp.realpath('.'), 
...
def write(x, results, color=random.choice(colors)):
    c1 = tuple(x[1:3].int())
    c2 = tuple(x[3:5].int())
    print(c1)
    print(c2)
